...
func main() {
	 ...
	 * sheet.  Will get deleted if read is successful.  This is necessary
	 * because you can't delete the last sheet in a spreadsheet.
	 *-------------------------------------------------------------------*/
	 ...
	 *-----------------------------------------------------*/
	q := "SELECT * FROM ovl_visits WHERE DATE(dateCreated) = CURDATE()"
	log.V(1).Printf("Query:%v\n", q)
